...
class Tools {
    ...
    * 求出给定时间所在周的起始时间和结束时间
    */
    public static function weekStarttime($time){
        $weektime = date('w',$time);
        if($weektime == 0){
            $weektime = 7;
        }
        $weektime = (int)$weektime;
        $nowtime = strtotime(date('Y-m-d', $time));

        $starttime = $nowtime - ($weektime - 1) * 86400;
        $endtime = $nowtime + (8 - $weektime) * 86400 - 1;
     ...
     */
    public static function getMoon(int $starttime, int $endtime, $dates = []){
        $days = date('t', $starttime);
        $date = date('Y-m', $starttime);

        $timeStart = strtotime($date . '-1 00:00:00');
        $timeEnd = strtotime($date . '-' . $days . ' 23:59:59');

        $dates[$timeStart] = $timeEnd;
        // 递归求出下一月的起始时间和截止时间
        if($timeEnd < $endtime){
            $dates = self::getMoon($timeEnd + 1, $endtime, $dates);
        }
        return $dates;
    }
